...
def borrarMedicamento(id_med):
    ...
    for Medicamento in Medicamentos:
        if int(id_med) != int(Medicamento.getId()):
...
def compraMed(name):
    ...
    for Medicamento in Medicamentos:
        if name == Medicamento.getNombre():
            ...
            Medicamento. setNumeroVendidos(Medicamento.getNumeroVendidos()+1)
            break
...
@app.route("/receta/<string:nombre>", methods=["POST"])
def receta(nombre):
    global Usuarios
    global Doctores
    global Enfermeras
    ...
    agregado = False
    nombre_doctor =""
    nombre_enfermedad = nombre.casefold()
    
    for doc in Doctores:
        if doctor == doc.getUser_name():
            nombre_doctor = doc.getNombre() + " " + doc.getApellido()
    
    for enf in Enfermedades:
        if nombre_enfermedad == enf.getNombre():
            agregado = True
    
    if agregado == True:
        for enfe in Enfermedades:
            if nombre_enfermedad == enfe.getNombre():
                enfe.setCantidad(enfe.getCantidad()+1)
             
    elif agregado == False:
        Enfermedades.append(Enfermedad(nombre_enfermedad,1))
